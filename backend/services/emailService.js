...
if (!hasSmtpConfig) {
    console.warn('⚠️  SMTP email fallback is not fully configured');
}
...
const sendWithSmtp = async (email, name, otp) => {
    if (!hasSmtpConfig) {
    ...
    const info = await transporter.sendMail({
        ...
        to: email,
        subject: 'Password Reset OTP - FreelanceCo',
...
const sendOTPEmail = async (email, name, otp) => {
    try {
        if (hasBrevoConfig) {
            ...
            sendSmtpEmail.htmlContent = buildOtpEmailHtml(name, otp);
        ...
        throw new Error('Unable to send OTP email. Configure Brevo sender or SMTP credentials.');
    }
};
